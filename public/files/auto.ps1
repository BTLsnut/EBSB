...
Set-Location -Path C:\Users\dkssh\Desktop\tt # set target path
$path=dir
...
foreach ($video in $videop)
{
    ...
    if ( -not (Test-Path $video)) {
        New-Item -Path . -Name $video  -ItemType "directory"
    }
    ...
    MP4Box -add $v_dir"\"$video"_1920.264" -fps 25.01 $v_dir"\"$video"_e_1920.mp4"
    MP4Box -add $v_dir"\"$video"_1080.264" -fps 25.01 $v_dir"\"$video"_e_1080.mp4"
    MP4Box -add $v_dir"\"$video"_720.264" -fps 25.01 $v_dir"\"$video"_e_720.mp4"
    MP4Box -add $v_dir"\"$video"_480.264" -fps 25.01 $v_dir"\"$video"_e_480.mp4"
